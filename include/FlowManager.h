...
struct FlowManager : RTMFP::Output, BandWriter {
	...
	virtual bool					onPeerHandshake70(const Base::SocketAddress& address, const Base::shared<Base::Buffer>& farKey, const std::string& cookie);
	...
	virtual void					removeHandshake(Base::shared<Handshake>& pHandshake) = 0;
	...
	const Base::shared<Base::Buffer>&	getNonce();
	...
	void									send(Base::shared<RTMFPSender>&& pSender);
...
protected:
	...
	// Create a new writer
	const Base::shared<RTMFPWriter>&		createWriter(const Base::Packet& signature, Base::UInt64 flowId=0);
	...
	virtual void				handleWriterException(Base::shared<RTMFPWriter>& pWriter) = 0;
	...
	// Job Members
	Base::shared<FlashConnection>					_pMainStream; // Main Stream (NetConnection or P2P Connection Handler)
	...
	Base::shared<Handshake>							_pHandshake; // Handshake object if not connected

	Base::Packet										_sharedSecret; // shared secret for crypted communication
	Base::shared<Base::Buffer>						_farNonce; // far nonce (saved for p2p group key building)
	Base::shared<Base::Buffer>						_nonce; // Our Nonce for key exchange, can be of size 0x4C or 0x49 for responder
...
private:
	...
	Base::shared<RTMFPWriter>&						writer(Base::UInt64 id, Base::shared<RTMFPWriter>& pWriter);
	...
	Base::shared<Base::Buffer>												_pBuffer; // buffer for sending packets
	Base::UInt32																_farId; // far id of the session
	Base::shared<RTMFPSender::Session>										_pSendSession; // session for sending packets
	Base::UInt16																_threadSend; // Thread used to send last message

	// writers members
	std::map<Base::UInt64, Base::shared<RTMFPWriter>>						_flowWriters; // Map of writers identified by id
