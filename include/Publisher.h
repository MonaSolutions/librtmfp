...
class Invoker;
class Listener;
struct Publisher : virtual Mona::Object {
	...
	ListenerType*				addListener(Mona::Exception& ex, const std::string& identifier, Args... args) {
		...
		if (it != _listeners.end() && it->first == identifier) {
			ex.set<Mona::Ex::Application>("Already subscribed to ", _name);
			return NULL;
		}
	...
	const Mona::Packet&		audioCodecBuffer() const { return _audioCodec; }
	const Mona::Packet&		videoCodecBuffer() const { return _videoCodec; }

	// Functions called by RTMFPSession
	void pushAudio(Mona::UInt32 time, const Mona::Packet& packet);
	void pushVideo(Mona::UInt32 time, const Mona::Packet& packet);
	void flush();
...
private:
	...
	Mona::Packet						_audioCodec;
	Mona::Packet						_videoCodec;
