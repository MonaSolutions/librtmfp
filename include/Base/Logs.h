...
struct Logs : virtual Static {
	...
	static bool			AddLogger(std::string&& name, Args&&... args) {
		...
		const auto& it = loggers.emplace(std::move(name), nullptr).first;
		if (it->second)
			return false;
	...
	static void			RemoveLogger(const std::string& name) { std::lock_guard<std::mutex> lock(_Mutex); GetLoggers().erase(name); }
	/*!
	Set LOG level */
...
private:
	...
	struct Loggers : std::map<std::string, unique<Logger>, String::IComparator>, virtual Object {
		Loggers() { self["console"].set<ConsoleLogger>(); }
		virtual ~Loggers() { std::lock_guard<std::mutex> lock(_Mutex); clear(); _failed.clear(); }
	...
	static struct Loggers&		GetLoggers() { static Loggers _Loggers; return _Loggers; }
...
#define DUMP(NAME,...) { if(Base::Logs::GetDump()) Base::Logs::Dump(NAME,__VA_ARGS__); }

#define DUMP_REQUEST(NAME, DATA, SIZE, ADDRESS) { if(Base::Logs::GetDump()) Base::Logs::DumpRequest(NAME, DATA, SIZE, NAME, " <= ", ADDRESS); }
#define DUMP_REQUEST_DEBUG(NAME, DATA, SIZE, ADDRESS) if(Logs::GetLevel() >= Base::LOG_DEBUG) DUMP_REQUEST(NAME, DATA, SIZE, ADDRESS)

#define DUMP_RESPONSE(NAME, DATA, SIZE, ADDRESS) { if(Base::Logs::GetDump()) Base::Logs::DumpResponse(NAME, DATA, SIZE, NAME, " => ", ADDRESS); }
