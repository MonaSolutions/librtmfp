...
struct SRT : virtual Object {
	...
	struct Socket : virtual Object, Base::Socket {
		Socket();
		virtual ~Socket();

		enum Type {
			TYPE_SRT = Base::Socket::TYPE_OTHER,
		};
		...
		virtual bool accept(Exception& ex, shared<Base::Socket>& pSocket);
	...
	private:
		...
		template <typename ...Args>
		static Exception& SetException(int error, Exception& ex, Args&&... args) { return Base::Socket::SetException(error, ex, std::forward<Args>(args)...);  }
	...
	struct Client : TCPClient {
		Client(IOSocket& io) : Base::TCPClient(io) {}

		SRT::Socket&	operator*() { return (SRT::Socket&)TCPClient::operator*(); }
		SRT::Socket*	operator->() { return (SRT::Socket*)TCPClient::operator->(); }
	private:
		shared<Base::Socket> newSocket() { return std::make_shared<SRT::Socket>(); }
	};

	struct Server : TCPServer {
		Server(IOSocket& io) : Base::TCPServer(io) {}
