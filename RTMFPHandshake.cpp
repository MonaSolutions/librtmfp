...
/* P2PStream */
RTMFPHandshake::P2PStream::P2PStream(const string& id) : peerId(id),
...
RTMFPHandshake::RTMFPHandshake(OnSocketError pOnSocketError) :
	...
	onPacket = [this](PoolBuffer& pBuffer, const SocketAddress& address) {
		...
		if (idStream > 0) {
			auto it = mapP2pPeers.find(address); // search P2P decoder
			pDecoder = (it == mapP2pPeers.end()) ? _pDecoder.get() : it->second.pDecoder.get();
		}
		...
		if (!pDecoder->process((UInt8*)pBuffer.data(), pBuffer.size()))
			...
			if (Logs::GetLevel() >= 7)
				DUMP("RTMFP", pBuffer.data(), pBuffer->size(), "Request from ", address.toString())
	...
	onErrorIn = [this](const Exception& ex) {
		onError(ex);
	};
	onPacketIn = [this](PoolBuffer& pBuffer, const SocketAddress& address) {
...
RTMFPHandshake::~RTMFPHandshake() {
	...
	if (_pSocketIn) {
		_pSocket->OnPacket::unsubscribe(onPacketIn);
		_pSocket->OnError::unsubscribe(onErrorIn);
		_pSocket->close();
	}
...
bool RTMFPHandshake::connect(Exception& ex, Invoker* invoker, const char* url, const char* host, const char* publication, bool isPublisher, HandshakeType type) {
	...
	INFO("Connecting to ", _hostAddress.host().toString(), ":", _hostAddress.port(), "(", (type == BASE_HANDSHAKE) ? "url" : "peer id" ," : ", url, ")...")
...
void RTMFPHandshake::manageHandshake(Exception& ex, BinaryReader& reader) {
...
void RTMFPHandshake::sendHandshake1(Exception& ex, BinaryReader& reader, UInt8 type) {
...
bool RTMFPHandshake::sendConnect(Exception& ex, BinaryReader& reader) {
	...
	_pSocketIn->OnError::subscribe(onErrorIn);
	_pSocketIn->OnPacket::subscribe(onPacketIn);
...
void RTMFPHandshake::p2pHandshake0(Exception& ex, BinaryReader& reader) {
	...
	if (peerIdSize != 0x22)
		...
		mapP2pPeers.emplace(_outAddress, peerId);
...
void RTMFPHandshake::p2pHandshake1(Exception& ex, BinaryReader& reader) {
	...
	auto it = mapP2pPeers.find(_outAddress);
	if (it == mapP2pPeers.end()) {
	...
	if (!computeKeys(ex, initiatorKey, initiatorNonce, writer.data()+noncePos, 0x49, it->second.pDecoder, it->second.pEncoder))
		return;
...
UInt8* RTMFPHandshake::packet() {
	if (!_pSender) {
		auto it = mapP2pPeers.find(_outAddress); // search P2P encoder
		_pSender.reset(new RTMFPSender(_pInvoker->poolBuffers, (it == mapP2pPeers.end()) ? _pEncoder : it->second.pEncoder));
	}
...
bool RTMFPHandshake::computeKeys(Exception& ex, const string& farPubKey, const string& initiatorNonce, const UInt8* responderNonce, UInt32 responderNonceSize, shared_ptr<RTMFPEngine>& pDecoder, shared_ptr<RTMFPEngine>& pEncoder) {
